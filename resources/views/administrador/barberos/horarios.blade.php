...
@section('title', 'Asignar horarios - BarberShop')
@section('page-title', 'Asignar horarios')
...
<section class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div>
            ...
            <p class="mt-2 text-sm text-ink-600">
                Selecciona los horarios de trabajo que tendrá asignados este barbero.
            </p>
        </div>
    ...
    <div class="rounded-panel border border-cream-200 bg-white shadow-card overflow-hidden">
        ...
        <form method="POST" action="{{ route('barbero.horarios.update', $barbero->id) }}">
            @csrf
            @method('PATCH')
            ...
            <div class="border-b border-cream-200 px-6 py-5">
                ...
                <p class="mt-1 text-sm text-ink-500">
                    Puedes asignar uno o varios horarios al barbero.
                </p>
            </div>
            ...
            <div class="p-6">
                ...
                @php
                    $horariosAsignados = collect(
                        old('horarios', $barbero->horarios->pluck('id')->toArray())
                        ...
                        @foreach ($horarios as $horario)
                            ...
                            <label class="block cursor-pointer">
                                ...
                                <input
                                    ...
                                    name="horarios[]"
                                    value="{{ $horario->id }}"
                                    {{ in_array($horario->id, $horariosAsignados) ? 'checked' : '' }}
                                    class="peer sr-only"
                                >
                                ...
                                <article class="rounded-panel border border-cream-200 bg-cream-50 p-5 transition-all hover:border-barber-red hover:bg-barber-red-50 peer-checked:border-barber-red peer-checked:bg-barber-red-50 peer-checked:shadow-panel">
                                    ...
                                    <div class="mt-5 flex items-center gap-2 text-sm font-bold text-barber-red">
                                        ...
                                        <span>
                                            {{ in_array($horario->id, $horariosAsignados) ? 'Horario seleccionado' : 'Seleccionar horario' }}
                                        </span>
                                    </div>
                ...
                @else
                    ...
                    <div class="rounded-card border border-warning bg-warning-light px-5 py-8 text-center">
                        ...
                        <p class="mt-2 text-sm font-medium text-ink-700">
                            Primero registra horarios activos para poder asignarlos a un barbero.
                        </p>
